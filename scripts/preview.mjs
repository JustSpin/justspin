...
const PGLITE_ASSET_FILES = ["pglite.data", "pglite.wasm", "initdb.wasm"];

/** Nitro bundles PGLite JS but drops the wasm/data files next to it. */
export function stagePgliteAssets(root = ROOT) {
  const srcDir = join(root, "node_modules/@electric-sql/pglite/dist");
  const destDir = join(root, ".vercel/output/functions/__server.func/_libs");
  if (!existsSync(destDir) || !existsSync(srcDir)) return { copied: 0 };
  let copied = 0;

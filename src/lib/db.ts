...
/**
 * Nitro's Vercel bundle inlines PGLite's JS but not pglite.data / *.wasm, so
 * the default locateFile path 404s under `vite preview`. Load the binaries
 * from node_modules (cwd is the app root) and pass them in. Deployed Neon
 * never calls this — DATABASE_URL is set.
 */
async function loadPgliteAssets(): Promise<{
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      if (!existsSync(wasmPath) || !existsSync(dataPath)) continue;
      const [wasm, data, initdb] = await Promise.all([
        readFile(wasmPath),
        readFile(dataPath),
      ...
      return {
        ...
        initdbWasmModule: initdb ? await WebAssembly.compile(initdb) : undefined,
        fsBundle: new Blob([new Uint8Array(data)]),
      };
    }
  } catch (err) {
    console.warn("[db] PGLite assets from disk failed, using package defaults:", err);
  }
  return {};
}
